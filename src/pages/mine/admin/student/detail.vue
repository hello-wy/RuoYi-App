<template>
  <scroll-view class="detail-page" scroll-y>
    ...
    <StudentEnrollmentPanel v-else-if="activeTab === 'school'" :student-id="studentId" />
    <StudentLearningPanel v-else-if="activeTab === 'study'" :student-id="studentId" />
    <StudentFollowUpPanel v-else-if="activeTab === 'visit'" :student-id="studentId" />
    <StudentSalonPurchasePanel v-else-if="activeTab === 'salon'" :student-id="studentId" />
    <StudentSituationPanel v-else-if="activeTab === 'situation'" :student-id="studentId" :situation="detail.studentSituation || ''" @saved="updateSituation" />
    <view v-else class="empty-card"><text class="empty-text">暂无内容</text></view>
  </scroll-view>
</template>
...
import StudentEnrollmentPanel from './StudentEnrollmentPanel.vue'
import StudentFollowUpPanel from './StudentFollowUpPanel.vue'
import StudentLearningPanel from './StudentLearningPanel.vue'
import StudentSalonPurchasePanel from './StudentSalonPurchasePanel.vue'
import StudentSituationPanel from './StudentSituationPanel.vue'
...
function updateSituation(studentSituation) {
  detail.value = { ...detail.value, studentSituation }
}
