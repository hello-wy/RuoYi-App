      ...
      <view v-else class="card-list">
        <view v-for="item in list" :key="item.id" class="lecture-card">
          ...
          <view class="card-main">
            ...
            <view class="card-actions"><button class="top-btn" size="mini" @click="handleToggleTop(item)">{{ item.isTop ? '取消置顶' : '置顶展示' }}</button><button class="edit-btn" size="mini" @click="openEdit(item.id)">编辑</button><button class="delete-btn" size="mini" @click="handleDelete(item)">删除</button></view>
          </view>
        </view>
...
import { delLectures, listLectures, updateLectures } from '@/api/system/lectures'
import { getLectureImageSrc } from '@/utils/lecture-cover'
import { requireAdminAccess } from '../access'
import { buildLectureListQuery, createLectureListState, sortLecturesByTop } from './helpers'
  ...
  methods: {
    ...
    async loadList(reset = false) {
      ...
      try {
        ...
        this.list = sortLecturesByTop(reset ? rows : this.list.concat(rows))
    ...
    async handleToggleTop(item) {
      const nextIsTop = item.isTop !== true
      await updateLectures({ id: item.id, isTop: nextIsTop })
      this.list = sortLecturesByTop(this.list.map(lecture => (
        lecture.id === item.id ? { ...lecture, isTop: nextIsTop } : lecture
      )))
      this.$modal.showToast(nextIsTop ? '已置顶展示' : '已取消置顶')
    },
...
.search-actions, .card-actions { display: flex; gap: 16rpx; margin-top: 20rpx; }.action-btn, .top-btn, .edit-btn, .delete-btn { margin: 0; font-size: 25rpx; }.action-btn { flex: 1; }.primary, .edit-btn { color: #fff; background: #5b4fd8; }.secondary { background: #eeeefa; color: #514b68; }.add { color: #fff; background: #0d9488; }
