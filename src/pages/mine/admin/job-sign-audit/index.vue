  ...
  <view class="audit-page">
    ...
    <scroll-view class="scroll-area" scroll-y @scrolltolower="loadMoreRecords">
      ...
      <view v-else class="card-list">
        <view v-for="item in records" :key="item.id || item.recordId || item.orderNo" class="audit-card">
          <view class="card-top">
          ...
          <view class="info-list">
            ...
            <view v-if="item.phoneMasked" class="info-row">
              <text class="info-label">手机号</text>
              <text class="info-value">{{ item.phoneMasked }}</text>
            </view>
            <view class="info-row">
              <text class="info-label">签到状态</text>
              <text class="info-value">{{ item.attendanceStatusLabel || '-' }}</text>
            </view>
            <view class="info-row">
            ...
            <view v-if="item.auditRemark" class="info-row remark-row">
              <text class="info-label">审核说明</text>
              <text class="info-value remark-text">{{ item.auditRemark }}</text>
            </view>
          </view>
          ...
          <view v-if="canAudit(item)" class="card-bottom">
            <button class="audit-btn reject" @click="handleAudit(item, 3)">驳回</button>
            <button class="audit-btn approve" @click="handleAudit(item, 2)">通过</button>
          </view>
        </view>
...
function getMaterialUrls(item) {
  return getJobSignAuditMaterialUrls(item).map(buildFullImageUrl).filter(Boolean)
}
...
function handleAudit(item, auditStatus) {
  uni.showModal({
    ...
    confirmColor: auditStatus === 2 ? '#0f766e' : '#dc2626',
    success: async (res) => {
